...
### DOM0级事件处理程序
使用DOM0级方法指定的事件处理程序被认为是元素的方法。因此，这时候的kgwr处理程序是在元素的作用域中运行；换句话说，程序中的this引用当前元素
...
### DOM2级事件处理程序
与DOM0方法一样，添加的事件处理程序也是在其依附的元素的作用域。优点是可以添加多个事件处理程序<br>
...
````
与图像不同只要设置了\<script>元素的src属性并将该元素添加到文档后，才会开始下载Javascript文件。换句话说，对于\<script>元素而言，指定src属性和指定事件处理程序的先后顺序就不重要了。
...
6. (我注：注意此时所有defer属性的脚本已经执行完毕)浏览器在`Document对象上触发DOMContentLoaded事件。`这标志着程序执行从同步脚本执行阶段转换到异步事件驱动阶段。但要注意，这时可能还有异步脚本没有执行完成。
7. 这时，文档已经完全解析完成，但是浏览器可能不在等待其他内容载入，如图片。当所有这些内容完成载入时，并且所有异步脚本完成载入和执行，`document.readyState属性改变为"complete"，Web浏览器触发Window对象上的load事件。`
...
![](https://ws2.sinaimg.cn/large/006tNc79ly1fnyxkk3cssj319x0ga148.jpg)

![](https://ws1.sinaimg.cn/large/006tNc79ly1fnyxmkq5c1j30y70jnn49.jpg)
